...
def renderButton(element):
    ...
    if data[3] in {"more", "drop-down", "info", "small", "view", "war-info", "profile", "play"}:
        ...
        try:
            img = c.image_store.get_sized_image("button " + data[3],
                                                'projects/' + c.project_name + '/images/' + data[3] + '.png',
        ...
        if data[7] == "icon left":
            ...
            if data[6] is not None:
                ...
                if data[4] != "":
                    ...
                    text_surf2.blit(text_surf,
                                    (icon.get_width(), text_surf2.get_height() // 2 - text_surf.get_height() // 2))
        ...
        elif data[7] == "icon top":

            # Icon
            if data[6] is not None:
                icon = c.image_store.get_image('projects/' + c.project_name + '/images/' + data[6] + '.png')
                h = img.get_height() - int(data[5]) * 2
                w = icon.get_width() / (icon.get_height() / h)
                icon = pygame.transform.smoothscale(icon, (w, h))
                img.blit(icon,(img.get_width()//2-w//2,10))
            ...
            if " " in data[4]:
                text_surf = font.renderText(line1, "default", int(data[5]))
                img.blit(text_surf,
                         (img.get_width() // 2 - text_surf.get_width() // 2,
                          img.get_height() - text_surf.get_height() * 1.7 - 5))

            text_surf = font.renderText(line2, "default", int(data[5]))
            img.blit(text_surf,
                     (img.get_width() // 2 - text_surf.get_width() // 2,
                      img.get_height() - text_surf.get_height() - 5))
    return img
